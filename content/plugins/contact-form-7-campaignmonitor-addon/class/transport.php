...
define('CF7CM_CS_REST_GET', 'GET');
define('CF7CM_CS_REST_POST', 'POST');
define('CF7CM_CS_REST_PUT', 'PUT');
define('CF7CM_CS_REST_DELETE', 'DELETE');
define('CF7CM_CS_REST_SOCKET_TIMEOUT', 1);

function CF7CM_CS_REST_TRANSPORT_get_available($requires_ssl, $log) {
    if(function_exists('curl_init') && function_exists('curl_exec')) {
        return new CF7CM_CS_REST_CurlTransport($log);
    } else if(CF7CM_CS_REST_TRANSPORT_can_use_raw_socket($requires_ssl)) {
        return new CF7CM_CS_REST_SocketTransport($log);
    } else { 
        $log->log_message('No transport is available', __FUNCTION__, CF7CM_CS_REST_LOG_ERROR);
...
function CF7CM_CS_REST_TRANSPORT_can_use_raw_socket($requires_ssl) {
...
class CF7CM_CS_REST_BaseTransport {
    
    var $_log;
    
    function CF7CM_CS_REST_BaseTransport($log) {
        $this->_log = $log;
    }
    ...
    function split_and_inflate($response, $may_be_compressed) {        
        ...
        if($may_be_compressed && preg_match('/^Content-Encoding:\s+gzip\s+$/im', $headers)) {        
            ...
            $this->_log->log_message('Inflated gzipped response: '.$original_length.' bytes ->'.
                strlen($result).' bytes', get_class(), CF7CM_CS_REST_LOG_VERBOSE);
        }
 ...
 * @since 1.0
 */
class CF7CM_CS_REST_CurlTransport extends CF7CM_CS_REST_BaseTransport {

    var $_curl_zlib;

    function CF7CM_CS_REST_CurlTransport($log) {
        $this->CF7CM_CS_REST_BaseTransport($log);
    ...
    function make_call($call_options) {
        ...
        switch($call_options['method']) {
            case CF7CM_CS_REST_PUT:
                curl_setopt($ch, CURLOPT_CUSTOMREQUEST, CF7CM_CS_REST_PUT);
                $headers[] = 'Content-Length: '.strlen($call_options['data']);
                curl_setopt($ch, CURLOPT_POSTFIELDS, $call_options['data']);
                break;
            case CF7CM_CS_REST_POST:
                curl_setopt($ch, CURLOPT_POST, true);
                curl_setopt($ch, CURLOPT_POSTFIELDS, $call_options['data']);
                break;
            case CF7CM_CS_REST_DELETE:
                curl_setopt($ch, CURLOPT_CUSTOMREQUEST, CF7CM_CS_REST_DELETE);
                break;
        }
        ...
        if(!$response && $response !== '') {
            $this->_log->log_message('Error making request with curl_error: '.curl_errno($ch),
                get_class($this), CF7CM_CS_REST_LOG_ERROR);
...
class CF7CM_CS_REST_SocketWrapper {
    var $socket;

    function open($domain, $port) {
        $this->socket = fsockopen($domain, $port, $errno, $errstr, CF7CM_CS_REST_SOCKET_TIMEOUT);

        if(!$this->socket) {
            die('Error making request with '.$errno.': '.$errstr);
            return false;
        } else if(function_exists('stream_set_timeout')) {
            stream_set_timeout($this->socket, CF7CM_CS_REST_SOCKET_TIMEOUT);
        }
...
class CF7CM_CS_REST_SocketTransport extends CF7CM_CS_REST_BaseTransport {

    var $_socket_wrapper;

    function CF7CM_CS_REST_SocketTransport($log, $socket_wrapper = NULL) {
        $this->CF7CM_CS_REST_BaseTransport($log);

        if(is_null($socket_wrapper)) {
            $socket_wrapper = new CF7CM_CS_REST_SocketWrapper();
        }
