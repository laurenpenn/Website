...
define('CF7CM_CS_REST_WRAPPER_VERSION', '1.0.10');

define('CF7CM_CS_REST_WEBHOOK_FORMAT_JSON', 'json');
define('CF7CM_CS_REST_WEBHOOK_FORMAT_XML', 'xml');
 ...
 *
 */
class CF7CM_CS_REST_Wrapper_Result {
    /**
    ...
    function CF7CM_CS_REST_Wrapper_Result($response, $code) {
 ...
 *
 */
class CF7CM_CS_REST_Wrapper_Base {
    /**
     ...
     * @var CF7CM_CS_REST_JsonSerialiser or CF7CM_CS_REST_XmlSerialiser
     * @access private
     */
    var $_serialiser;
     ...
     * @var CF7CM_CS_REST_CurlTransport or CF7CM_CS_REST_SocketTransport or your own custom transport.
     * @access private
     */
    var $_transport;
     ...
     * @param $debug_level int The level of debugging required CF7CM_CS_REST_LOG_NONE | CF7CM_CS_REST_LOG_ERROR | CF7CM_CS_REST_LOG_WARNING | CF7CM_CS_REST_LOG_VERBOSE
     * @param $host string The host to send API requests to. There is no need to change this
     * @param $log CF7CM_CS_REST_Log The logger to use. Used for dependency injection
     * @param $serialiser The serialiser to use. Used for dependency injection
     * @param $transport The transport to use. Used for dependency injection
     * @access public
     */
    function CF7CM_CS_REST_Wrapper_Base(
        $api_key,
        $protocol = 'https',
        $debug_level = CF7CM_CS_REST_LOG_NONE,
        $host = 'api.createsend.com',
        $log = NULL,
        $serialiser = NULL,
        $transport = NULL) {
            
        $this->_log = is_null($log) ? new CF7CM_CS_REST_Log($debug_level) : $log;
            
        $this->_protocol = $protocol;
        $this->_base_route = $protocol.'://'.$host.'/api/v3/';

        $this->_log->log_message('Creating wrapper for '.$this->_base_route, get_class($this), CF7CM_CS_REST_LOG_VERBOSE);

        $this->_transport = is_null($transport) ?
            CF7CM_CS_REST_TRANSPORT_get_available($this->is_secure(), $this->_log) :
            $transport;

        $transport_type = method_exists($this->_transport, 'get_type') ? $this->_transport->get_type() : 'Unknown';
        $this->_log->log_message('Using '.$transport_type.' for transport', get_class($this), CF7CM_CS_REST_LOG_WARNING);

        $this->_serialiser = is_null($serialiser) ?
            CF7CM_CS_REST_SERIALISATION_get_available($this->_log) : $serialiser;
            
        $this->_log->log_message('Using '.$this->_serialiser->get_type().' json serialising', get_class($this), CF7CM_CS_REST_LOG_WARNING);

        $this->_default_call_options = array (
            'credentials' => $api_key.':nopass',
            'userAgent' => 'CF7CM_CS_REST_Wrapper v'.CF7CM_CS_REST_WRAPPER_VERSION.
    ...
    function put_request($route, $data, $call_options = array()) {
        return $this->_call($call_options, CF7CM_CS_REST_PUT, $route, $data);
    }
    
    function post_request($route, $data, $call_options = array()) {
        return $this->_call($call_options, CF7CM_CS_REST_POST, $route, $data);
    }
    
    function delete_request($route, $call_options = array()) {
        return $this->_call($call_options, CF7CM_CS_REST_DELETE, $route);
    }
    
    function get_request($route, $call_options = array()) {
        return $this->_call($call_options, CF7CM_CS_REST_GET, $route);
    }
    ...
    function _call($call_options, $method, $route, $data = NULL) {
        ...
        $this->_log->log_message('Making '.$call_options['method'].' call to: '.$call_options['route'], get_class($this), CF7CM_CS_REST_LOG_WARNING);
            
        $call_result = $this->_transport->make_call($call_options);

        $this->_log->log_message('Call result: <pre>'.var_export($call_result, true).'</pre>',
            get_class($this), CF7CM_CS_REST_LOG_VERBOSE);

        if($call_options['deserialise']) {
            $call_result['response'] = $this->_serialiser->deserialise($call_result['response']);
        }
         
        return new CF7CM_CS_REST_Wrapper_Result($call_result['response'], $call_result['code']);
    }
}