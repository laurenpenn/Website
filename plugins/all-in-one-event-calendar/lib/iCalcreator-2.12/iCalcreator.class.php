 ...
 */
  function parse( $unparsedtext=FALSE ) {
    ...
    $rows = preg_replace( '/\R |\R\t/', '', $rows );
    // convert all line endings to $nl value that defaults to \r\n
    $rows = preg_replace('/\R/', $nl, $rows );
    $rows = explode( $nl, $rows );
    /* skip beginning empty lines */
    $lix = 0;
    while( empty( $rows[$lix] ) && ( ( count( $rows ) - $lix ) >= 0 ))
      $lix += 1;
    /* identify BEGIN:VCALENDAR, MUST be first row */
    if( 'BEGIN:VCALENDAR' != strtoupper( substr( $rows[$lix], 0, 15 )))
      return FALSE;                   /* err 8 */
    if( 3 > count( $rows ))
      return FALSE;                   /* err 10 */
    $comp    = & $this;
