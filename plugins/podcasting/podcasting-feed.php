...
class PodcastingFeed {
	...
	function addItunesFeed() {
		if ( 'podcast' == get_query_var('feed') ) {
			...
			// iTunes new feed url (used to move podcast feed)
			if ( '' != get_option('pod_itunes_new-feed-url') )
				echo '<itunes:new-feed-url>' . $this->getOption('pod_itunes_new-feed-url') . '</itunes:new-feed-url>' . "\n	";
	...
	function addItunesItem() {
      ...
      if($itunes_summary == "")
      {
          ob_start(); the_content(); $itunes_summary = ob_get_contents(); ob_end_clean();
			...
			$itunes_summary = $this->limitStringLength($this->flatTextEncode(strip_tags($itunes_summary)), 4000);
