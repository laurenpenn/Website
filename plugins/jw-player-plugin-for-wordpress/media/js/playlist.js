...
function fileDialogStart() {
	jQuery("#playlist-upload-error").empty();
}

// progress and success handlers for playlist multi uploads
function fileQueued(fileObj) {
	// Get rid of unused form
	jQuery('.playlist-blank').remove();
	// Collapse a single item
	if ( jQuery('form.type-form #playlist-items').children().length == 1 && jQuery('.hidden', '#playlist-items').length > 0 ) {
		jQuery('.describe-toggle-on').show();
		jQuery('.describe-toggle-off').hide();
		jQuery('.slidetoggle').slideUp(200).siblings().removeClass('hidden');
	}
	// Create a progress bar containing the filename
	jQuery('#playlist-items').append('<div id="playlist-item-' + fileObj.id + '" class="playlist-item child-of-' + post_id + '"><div class="progress"><div class="bar"></div></div><div class="filename original"><span class="percent"></span> ' + fileObj.name + '</div></div>');
	// Display the progress div
	jQuery('.progress', '#playlist-item-' + fileObj.id).show();
...
function uploadProgress(fileObj, bytesDone, bytesTotal) {
	...
	var w = jQuery('#playlist-items').width() - 2, item = jQuery('#playlist-item-' + fileObj.id);
...
function preparePlaylistItem(fileObj, serverData) {
	var f = ( typeof shortform == 'undefined' ) ? 1 : 2, item = jQuery('#playlist-item-' + fileObj.id);
	...
	if ( isNaN(serverData) || !serverData ) {
		...
		preparePlaylistItemInit(fileObj);
	}
	// New style: server data is just the attachment ID, fetch the thumbnail and form html from the server
	else {
		item.load('async-upload.php', {attachment_id:serverData, fetch:f}, function(){preparePlaylistItemInit(fileObj);updatePlaylistForm()});
	}
}
...
function preparePlaylistItemInit(fileObj) {
	...
	jQuery('#playlist-item-' + fileObj.id + '.startopen').removeClass('startopen').slideToggle(500).siblings('.toggle').toggle();
}

function itemAjaxError(id, html) {
	var item = jQuery('#playlist-item-' + id);
	...
	item.html('<div class="error-div">'
				...
				+ html
				+ '</div>');
	item.find('a.dismiss').click(function(){jQuery(this).parents('.playlist-item').slideUp(200, function(){jQuery(this).remove();})});
}
...
function deleteSuccess(data, textStatus) {
	...
	var id = this.id, item = jQuery('#playlist-item-' + id);
...
function updatePlaylistForm() {
	var one = jQuery('form.type-form #playlist-items').children(), items = jQuery('#playlist-items').children();
	...
	if ( items.not('.playlist-blank').length > 0 )
...
function uploadSuccess(fileObj, serverData) {
	// if async-upload returned an error message, place it in the playlist item div and return
	if ( serverData.match('playlist-upload-error') ) {
		jQuery('#playlist-item-' + fileObj.id).html(serverData);
		return;
	}

	preparePlaylistItem(fileObj, serverData);
	updatePlaylistForm();
...
function wpQueueError(message) {
	jQuery('#playlist-upload-error').show().text(message);
}
...
function uploadError(fileObj, errorCode, message) {
	...
	switch (errorCode) {
		...
		case SWFUpload.UPLOAD_ERROR.FILE_CANCELLED:
			jQuery('#playlist-item-' + fileObj.id).remove();
			break;
		default:
...
jQuery(document).ready(function($){
	$('input[type="radio"]', '#playlist-items').live('click', function(){
	...
	$('button.button', '#playlist-items').live('click', function(){
