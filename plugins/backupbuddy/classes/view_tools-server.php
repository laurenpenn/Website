...
function ini_get_bool( $a ) {
	...
	if ( version_compare( PHP_VERSION, $this->_parent->_php_minimum, '<=' ) ) {
		$this_test['status'] = __('FAIL', 'it-l10n-backupbuddy');
	} else {
	...
	if ( $this->_var == 'pluginbuddy_backupbuddy' ) {
		if ( !file_exists( $this->_options['backup_directory'] ) ) {
			if ( $this->_parent->mkdir_recursive( $this->_options['backup_directory'] ) === false ) {
				$this->alert( sprintf( __('Unable to create backup storage directory (%s)', 'it-l10n-backupbuddy') , $this->_options['backup_directory'] ), true, '9002' );
				return false;
			}
		}
	}
	...
	if ( file_exists( ABSPATH . '.htaccess' ) ) {
		...
		if ( !stristr( $htaccess_contents, 'addhandler' ) ) {
			$this_test['status'] = __('OK', 'it-l10n-backupbuddy');
			$this_test['value'] = __('n/a', 'it-l10n-backupbuddy');
		} else {
			$this_test['status'] = __('WARNING', 'it-l10n-backupbuddy');
			$this_test['value'] = __('exists', 'it-l10n-backupbuddy');
		}
...
if ( isset( $_GET['phpinfo'] ) && $_GET['phpinfo'] == 'true' ) {
	...
	echo '<br>';
	echo '<center>';
	echo '<a href="' . $this->_selfLink . '-tools&phpinfo=true" class="button secondary-button" style="margin-top: 3px;">'. __('Display Extended PHP Settings via phpinfo()', 'it-l10n-backupbuddy') . '</a>';
	echo '</center>';
