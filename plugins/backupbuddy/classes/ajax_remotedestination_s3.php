...
<form id="posts-filter" enctype="multipart/form-data" method="post" action="<?php echo admin_url('admin-ajax.php') . '?action=pb_backupbuddy_remotedestination&edit=' . htmlentities( $edit_id ); ?>&callback_data=<?php if ( !empty( $_GET['callback_data'] ) ) { echo $_GET['callback_data']; } ?>#pb_backupbuddy_tab_s3">
	<div class="tablenav">
...
?>
	...
	<table class="form-table">
		...
		<tr>
			<td><label for="directory"><?php _e('Directory (optional)', 'it-l10n-backupbuddy'); $this->tip( __('[Example: backupbuddy] - Directory name to place the backup in within the bucket.', 'it-l10n-backupbuddy') ); echo ' '; $this->video( 'njT1ExMgUrk#20', __('Create an Amazon S3 bucket', 'it-l10n-backupbuddy') ); ?></label></td>
