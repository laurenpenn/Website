 ...
 * Text Domain: it-l10n-backupbuddy 
 *
 * Installation:
 * 
...
if (!class_exists("pluginbuddy_backupbuddy")) {
	class pluginbuddy_backupbuddy {
		var $_version = '2.2.6';									// Deprecated variable. Use $this->plugin_info( 'version' ).
		...
		function save() {
			...
			$this->update_option($this->_var, $this->_options);
			return true;
		}
			...
			if ( preg_match( '/(\d+)(\w*)/', $current_memory_limit, $matches ) ) {
				...
				if ( 'g' !== strtolower( $unit ) ) {
					if ( ( $current_memory_limit < 256 ) || ( 'm' !== strtolower( $unit ) ) )
						...
						//$this->status( 'details', __('Set memory limit to 256M. Previous value < 256M.', 'it-l10n-backupbuddy') );
				}
			} else {
				// Couldn't determine current limit, set to 256M to be safe.
				@ini_set('memory_limit', '256M');
				//$this->status( 'details', __('Set memory limit to 256M. Previous value unknown.', 'it-l10n-backupbuddy') );
			}
		}
		...
		function action_post_updated_messages( $messages ) {
			...
			$backup_message = ' | <a href="admin.php?page=pluginbuddy_backupbuddy-backup&run_backup=full">' . __('Full Backup', 'it-l10n-backupbuddy') . '</a> | <a href="admin.php?page=pluginbuddy_backupbuddy-backup&run_backup=db">' . __('Database Backup', 'it-l10n-backupbuddy') . '</a>';
