...
$strShowTableDimension = 'テーブルの大きさを表\示する';
$strShowTables = 'テーブルを表\示する';
$strShowThisQuery = ' 実行したクエリをここに表\示する';
...
$strTraditionalSpanish = 'スペイン語 (伝統表\記)';
$strTraffic = 'トラフィック';
$strTransactionCoordinator = 'トランザクションコーディネータ';
$strTransformation_application_octetstream__download = 'フィールドのバイナリデータをダウンロードするリンクを表\示します。1 つ目のオプションにバイナリファイルのファイル名を指定することもできますし、2 つ目のオプションにファイル名を保持しているフィールドの名前を渡すこともできます。2 つ目のオプションを使う場合は 1 つ目のオプションは空文字列にする必要があります';
$strTransformation_application_octetstream__hex = 'データを 16 進数で表\記します。オプションとして最初のパラメータにスペースを追加する頻度を指定することもできます (デフォルトは 2 ニブルごとです)';
$strTransformation_image_jpeg__inline = 'クリック可能なサムネイルを表\示します; オプションとして幅と高さの最大値をピクセル単位で指定できます。アスペクト比はそのまま維持されます';
$strTransformation_image_jpeg__link = 'この画像をダウンロードするためのリンクを表\示します';
$strTransformation_image_png__inline = 'image/jpeg: inlineをご覧ください';
$strTransformation_text_plain__dateformat = 'TIME、TIMESTAMP、DATETIME、あるいはUnixタイムスタンプの値が入っているフィールドを書式にあわせて表\示します。1 つ目のオプションはタイムスタンプに加算する時差です(デフォルト: 0)。2 つ目のオプションには日時の書式文字列を指定します。3 つ目のオプションはローカルの日時とみなすか UTC とみなすかです ("local"ないし"utc"という文字列を指定してください)。この値によって日時の表\記が変わります。"local" の場合は PHP の strftime() 関数の説明をご覧ください。"utc" の場合は gmdate() を利用します'; // XXX replaced
...
$strTransformation_text_plain__formatted = 'フィールドの内容をそのまま、htmlspecialchars()を通さずに表\示します。この場合、フィールドには正しい HTML が含まれているものと仮定します';
$strTransformation_text_plain__imagelink = '画像とリンクを表\示します。フィールドにはファイル名を入れてください。1 つ目のオプションは "http://domain.com/" のようなURLの先頭部分です。2 つ目と3 つ目のオプションには幅と高さをピクセル単位で指定します';
$strTransformation_text_plain__link = 'リンクを表\示します。フィールドにはファイル名を入れてください。1 つ目のオプションは "http://domain.com/" のような URL の先頭部分、2 つ目のオプションはリンクのタイトルです';
$strTransformation_text_plain__sql = 'テキストの内容を SQL クエリとみなし、構文ハイライト表\示します';
$strTransformation_text_plain__substr = '文字列の一部のみを表\示します。1 つ目のオプションは文字列の先頭から何文字スキップするか (デフォルトは 0 です)、2 つ目のオプションは表\示するテキストの文字数です (デフォルトは文字列の末尾までです)。3 つ目のオプションは文字列を切り詰めるときに前／後に補う省略記号です (デフォルトは ... です)';
$strTruncateQueries = 'クエリの表\示を切り詰める';
$strTurkish = 'トルコ語';
$strType = '種別';
...
$strViewDumpDatabases = 'データベースのダンプ(スキーマ)表\示';
$strViewDumpDB = 'DB のダンプ(スキーマ)表\示';
$strViewDump = 'テーブルのダンプ(スキーマ)表\示';
